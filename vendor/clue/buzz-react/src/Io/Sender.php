...
class Sender
{
    ...
    public static function createFromLoopConnectors(LoopInterface $loop, ConnectorInterface $connector, ConnectorInterface $secureConnector = null)
    {
        ...
        if ($ref->getConstructor()->getNumberOfRequiredParameters() == 1) {
            // react/http-client:0.5 switch to ConnectorPair
            $connectorPair = new ConnectorPair($connector, $secureConnector);
            $http = new HttpClient($connectorPair);
        } else if ($ref->getConstructor()->getNumberOfRequiredParameters() == 2) {
    ...
    public static function response(ResponseStream $response, $deferred, $requestStream)
    {
        $bodyBuffer = '';
    ...
    public function send(Request $request)
    {
        ...
        $requestStream->on('redirect', function (ResponseStream $response) use ($deferred, $requestStream) {
            self::response($response, $deferred, $requestStream);
        });
